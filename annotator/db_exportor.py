...
from annotator.query_util import get_question_list, get_post_text, \
    ...
    POST_TEXT_TYPE_TOKENIZE_WITH_SMALL_CODE_BLOCK
...
def export_only_label_large_code_block_ner_data_set_by_step(
        start=0,
        max_num=1000,
        step=100,):
    for offset in range(start, max_num, step):
        export_only_label_large_code_block_ner_data_set(num=step, offset=offset)
        print 'done', offset, '-', offset + step
...
def export_only_label_large_code_block_ner_data_set(
        num=100,
        offset=0,
        output_file_path=".",
        output_file_name="large_code_block_ner.txt"):
    output_file_name = '{0}_{1}_{2}_{3}'.format(str(offset), str(offset + num),
                                                '_only_label_large_code_block_', output_file_name)
            ...
            with codecs.open(os.path.join(output_file_path, dev_output_file_name), 'w', encoding='utf-8') as dev_output:
                ...
                for question in question_list:
                    ...
                    if question.title:
                        ...
                        words_list.append(title_words)
                        tags_list.append(title_tags)
                        get_large_code_block_tag_for_one_post(question, tags_list, words_list)
                    ...
                    if answer_list:
                        ...
                        answer_count = 0
                    for answer in answer_list:
                        get_large_code_block_tag_for_one_post(answer, tags_list, words_list)
...
def get_large_code_block_tag_for_one_post(post, tags_list, words_list):
    post_text = get_post_text(post, POST_TEXT_TYPE_TOKENIZE_WITH_SMALL_CODE_BLOCK)
    if post_text:
        code_block_list=get_all_code_block(
            post.id, CodeBlockWithTokenizeCode.LARGE_CODE_BLOCK)
        if code_block_list:
            new_words_list, new_tags_list = build_code_tagger_data_for_whole_post(post_text,
                                                                              get_all_code_block(
                                                                                  post.id, CodeBlockWithTokenizeCode.LARGE_CODE_BLOCK))
